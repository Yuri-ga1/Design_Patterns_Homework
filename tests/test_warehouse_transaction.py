...
class TestWarehouseTransaction(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.nomenclature = Nomenclature.create(name="Test Nomenclature", full_name="Test Full Nomenclature", group=NomenclatureGroup.default_group_cold(), unit=MeasurementUnit.default_unit_gramm())
        self.unit = MeasurementUnit.create(name="Test Unit")
        self.count = 10.
    ...
    def test_equality(self):
        ...
        transaction1 = WarehouseTransaction.create(warehouse=self.warehouse, nomenclature=self.nomenclature, count=10., unit=self.unit, period=self.period)
        transaction2 = WarehouseTransaction.create(warehouse=self.warehouse, nomenclature=self.nomenclature, count=10., unit=self.unit, period=self.period)
